...
5. [Anywhere Wins](#5-anywhere-wins)
...
| Property | Value |
|----------|-------|
| **Game Type** | Pay-anywhere (Sweet Bonanza style) |
| **Grid Size** | 6 reels × 5 visible rows |
| **RTP** | 96.2% |
| **Volatility** | Medium to Very High |
| **Max Win** | 25,000× bet |
| **Anywhere Minimum** | 8+ matching symbols |
...
- Positions in events use **visible row indices (1-5)** unless otherwise specified
...
### Rules
1. **Minimum 8** matching symbols anywhere on the grid form a win (positioning/adjacency does **not** matter).
2. Scatters (`S`, `BS`) do NOT substitute – they only trigger bonuses/payouts listed in the scatter table.

### Example Anywhere Win
```
Total H1 symbols visible: 9 (spread arbitrarily across the 6×5 board)
→ Pays the 8-9 tier for H1 (10× bet)
```
...
- Final tumble win = sum of all individual symbol wins in that spin
...
2. If the tumble produces any win amount, **all bombs on the board activate**, even if they were not part of the symbol set that triggered the win
3. **Multiple bombs ADD together** before multiplying
   - Example: 5× bomb + 10× bomb = 15× total multiplier
4. Tumble win × (sum of bomb multipliers) = final tumble win for that tumble
...
### Important Notes
- Bombs activate whenever a tumble produces a win, even if they weren’t in the symbol set that hit
